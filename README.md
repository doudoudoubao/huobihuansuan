...
| `18x12 欧元 人民币` | 先算式后换算，`x` `×` `*` `➗` 都认 |
| `100除以4 usd cny` / `100加50 usd cny` | 中文写的运算词也行 |
| `(23.5+40)*3 eur cny` / `（10+2）×5 usd cny` | 括号、全角符号照样算 |
| `1.5k usd` / `10万日元` / `2亿越南盾` | k / w / m / b / 千 / 万 / 亿 |
| `1万5 日元` / `1亿2千万 日元` | 中文连写：15000、120000000 |
...
- **法币**：Yahoo → 新浪财经 → fxratesapi（均为准实时）→ Frankfurter/ECB → open.er-api → currency-api
  - **新浪财经**是准实时法币的主力：免鉴权、秒级更新，而且**一次请求就能拿
    二十多个货币对**，天然不会撞限流。字段位置是照生产环境的真实响应对出来的，
    并用「最新价必须落在当日高低区间内」做校验 —— 万一哪天字段挪位，
    宁可这条不出数，也不发出错的汇率
  - **Yahoo** 每个符号都要单独请求，且对没有会话凭证的请求一律回 429
    （跟频率无关，是反爬策略）。所以先取 cookie + crumb 再请求；
    每轮只刷 6 个「最久没更新」的符号、并发 2、请求间留间隔，轮转覆盖；
    撞到限流就作废凭证、冷却 10 分钟吃缓存。IP 一旦被拉黑就连 crumb 都拿不到，
    这时它会一直是黄色的备用状态，不影响换算
...
页脚会**如实说明这个数有多新**，三种情况分得很清楚：

| 页脚 | 含义 |
| --- | --- |
| `📡 yahoo · 刚刚` | 准实时源，分钟级刷新 |
| `🗓 frankfurter · 每日更新 · 昨日报价` | 走的是欧洲央行这类每日源。**这是正常的**，ECB 一天只公布一次价，刷新也不会变新 |
| `⚠️ 数据源连不上，已 4 小时没刷新成功` | 真出问题了：抓取通道断了，该去 `/status` 看看 |

「报价本身多老」和「我们多久没抓到新数据」是两件事 —— 只有后者才值得警告。
...
> 5. 看自检结果决定要不要处理：
>    - ○ 黄色标记的源挂了但同类还有源顶着 —— **忽略即可，这是正常的**
>    - ✗ 红色标记，或结论行写「无源可用」 —— 需要处理，按它给的提示排查
...
```
  ...
  ✓  汇率源    6/7 在线
      ● binance         31 种货币
      ● okx             28 种货币
      ○ coingecko     api.coingecko.com 请求过频被限流 (429)（有备用顶着，忽略即可）
      ● frankfurter     31 种货币
      ● open-er-api    163 种货币
      ● yahoo           22 种货币
      ● currency-api   140 种货币
      └ 法币 → yahoo  准实时，另有 3 个备用
      └ 加密 → binance  准实时，另有 3 个备用
  ✓  试算      1 USD = 7.2431 CNY   (binance)
  ────────────────────────────────────────────
  一切正常，可以 python run.py 启动了
```

**怎么看这份报告**：不用数「几个源是绿的」，只看 `└` 开头的那两行结论 ——
只要法币和加密各自都有一个准实时主力，就没问题。

| 标记 | 含义 | 要不要管 |
| --- | --- | --- |
| ● 绿 | 这个源正常供数 | 不用管 |
| ○ 黄 | 挂了，但同类还有源顶着 | **不用管**，备胎没上场而已 |
| ✗ 红 | 挂了，而且没有源能接替 | 要管 |
| └ 只剩每日更新的源 | 还能换算，但汇率一天才动一次 | 要管，见下 |
| └ 无源可用 | 这一类换算会直接失败 | 必须管 |

终端不支持颜色（重定向到文件、CI 日志、设了 `NO_COLOR`）时自动退化成纯文本，
不会留一堆转义序列。
...
仓库里带了 `update.sh`，自动判断你用的是 Docker 还是 venv：

```bash
./update.sh              # 拉代码 → 装依赖 → 自检 → 重启
./update.sh -y           # 不交互，一路默认继续（给 AI / CI / cron 用）
./update.sh --check-only # 只拉代码和自检，先看看再决定要不要重启
./update.sh --rollback   # 出问题了，回到更新前那个版本
```

自检退出码 `2`（能跑但数据源有缺口，比如某个源被限流）**不会**拦住重启 ——
那不是故障。只有退出码 `1`（起不来）才会停下来问你。

它会做这些事：先看工作区有没有未提交的改动（有就先问你），记下当前版本号，
`git pull` 之后列出这一次更新了哪些内容，装依赖，跑一遍 `--check`，
**自检不过会先停下来问你**（Docker 模式下旧容器还在跑，服务不中断），
确认后才重启。
...
> 帮我更新服务器上的 Telegram 货币换算机器人。
>
> 1. 找到仓库目录（一般叫 `huobihuansuan`，`cd` 进去）
> 2. 跑 `./update.sh -y`（加 `-y` 才不会卡在交互提示上）。
>    它会自动判断是 Docker 还是 venv 部署，
>    依次做：查工作区 → git pull → 装依赖 → 跑自检 → 重启
> 3. 自检退出码：0 = 正常，1 = 起不来，2 = 能跑但数据源有缺口
> 4. 自检里 ○ 黄色标记的数据源挂了但同类还有源顶着 —— **正常现象，不用管**；
>    只有 ✗ 红色标记、或结论行写「无源可用 / 只剩每日更新的源」才需要处理
> 5. 更新完确认服务起来了：
>    Docker 看 `docker compose logs -f`，systemd 看 `journalctl -u huobihuansuan -f`，
>    日志里应该有「已登录为 @xxx」
> 6. **要是更新后起不来，先跑 `./update.sh --rollback` 回到上一版**，
>    确保服务先恢复，然后再看日志排查
>
> 注意：`.env` 和 `data/` 不在 git 里，不要动它们，也不要把 token 打印出来。
> 数据库有新字段会在启动时自动补列，不需要你手动迁移。
> 脚本会问「是否继续 / 是否重启」，按实际情况回答就行。
...
# venv
git pull && pip install -r requirements.txt && sudo systemctl restart huobihuansuan
```

**几件不用操心的事**：

- `.env` 和 `data/` 都不在 git 里，`git pull` 不会碰它们
- 数据库有新字段时，启动时会自动补列，不用手动迁移
- 你的用户设置（常用币种、提醒、订阅）全在 `data/bot.db` 里，原样保留

**回滚**：`./update.sh --rollback` 会停在更新前那个提交上（detached HEAD）。
想再回到最新版，`git checkout <你的分支> && ./update.sh` 即可，
脚本会把分支名一起记下来并提示你。
...
| 更新后起不来 | `./update.sh --rollback` 先回到上一版，再看日志排查 |
| `update.sh` 提示「不在任何分支上」 | 上次回滚过，`git checkout <你的分支>` 切回来即可 |
