...
一个「不用记命令」的实时货币换算机器人：直接把 `100 usd cny`、`100美元换人民币`、
`$100`、`(23.5+40)*3 eur cny` 发给它就行。多数据源聚合、自动故障转移，
法币约每分钟、加密货币约每 15 秒刷新一次。
...
结果卡片下方自带按钮：**⇄ 反向 · 🔄 刷新 · 📈 走势 · 🔔 提醒**，
再加一排你的常用币种，点一下就换一个目标货币。
...
```
/setbase CNY               默认币种
/fav USD EUR JPY HKD       收藏币种（决定「一次多换」列出哪些）
...
| `DEFAULT_FAVORITES` | `USD,EUR,JPY,HKD,GBP` | 新用户的收藏列表 |
